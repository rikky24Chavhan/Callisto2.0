...
class RealmMealDataController: MealDataController {
    ...
    let imageFileManager: FileManager
    let apiClient: PilotAPIClient
    let imageUploadSession: URLSession

    private let bag = DisposeBag()

    init(
        realmConfiguration: Realm.Configuration = Realm.Configuration(deleteRealmIfMigrationNeeded: true),
        imageFileManager: FileManager = .default,
        apiClient: PilotAPIClient,
        imageUploadSession: URLSession = URLSession(configuration: .default)) throws {
                    ...
                    do {
                        ...
                        try realm.write {
                            mealEvents.forEach { mealEvent in
                                ...
                                realm.add(mealEvent, update: true)
                            }
                        }
                        completion?(.success(()))
                    } catch(let error) {
    ...
    func saveMealEvent(_ mealEvent: MealEvent, completion: ((SaveResult<MealEvent>) -> Void)?) {
        ...
        let apiCallCompletion: ((Result<RealmMealEvent>) -> Void) = { [weak self] result in
            guard let welf = self else {
                return
            }
    ...
    private func performSaveMealEventAPICall(mealEvent: RealmMealEvent, completion: ((Result<RealmMealEvent>) -> Void)?) {
            ...
            case .success(let mealEvents):
                ...
                do {
                    try welf.realm.write {
                        welf.realm.add(newMealEvent, update: true)
                    }
                } catch {
    ...
    func getMeals(forClassification classification: MealClassification, completion: ((Result<Void>) -> Void)?) {
        apiClient.getMeals(forClassification: classification) { [weak self] result in
            ...
            case .success(let meals):
                try! welf.realm.write {
                    welf.realm.add(meals, update: true)
                }
                ...
                if !mealsHidden.isEmpty {
                    try! welf.realm.write {
                        ...
                        welf.realm.add(mealsHidden, update: true)
                    }
                }
    ...
    func asyncSave<T: Object>(_ object: T, completion: ((Result<T>) -> Void)?) {
        do {
            try realm.write {
