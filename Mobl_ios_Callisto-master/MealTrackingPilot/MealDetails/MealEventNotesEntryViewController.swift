...
class MealEventNotesEntryViewController: UIViewController, UITextViewDelegate {
    ...
    private func setupDeleteBarButtonItem() {
        ...
        deleteBarButtonItem.setTitleTextAttributes(
            [NSAttributedStringKey.font : UIFont.openSansSemiboldFont(size: 17.0),
             NSAttributedStringKey.foregroundColor : UIColor.piDarkishPink],
            for: .normal)
    ...
    @objc private func keyboardWillChange(_ notification: Notification) {
        guard let keyboardHeight = (notification.userInfo?[UIKeyboardFrameEndUserInfoKey] as? CGRect)?.size.height else {
            return
        }

        textContainerBottomConstraint.constant = keyboardHeight - textView.customReturnButtonAccessoryViewHeight

        let duration = (notification.userInfo?[UIKeyboardAnimationDurationUserInfoKey] as? TimeInterval) ?? 0.0
