...
final class PilotLoginClient: AccessCredentialProviding, LoginUserProviding, LoginClientDelegate, LoginClientUIDelegate, AccessCredentialsLoginClient, LoginCredentialsLoginClient {
    ...
    private var refreshLoginCompletion: ((Result<AccessCredentials>) -> Void)?
    ...
    fileprivate func sendLoginRequest(credentials: LoginCredentials, completion: ((Result<PilotLoginResponse>) -> Void)?) {
        let request = PilotRequest.login(credentials: credentials).urlRequest
        apiClient.sendRequest(request, completion: completion)
    }

    private func handleLoginResult(_ result: Result<AccessCredentials>) {
        switch result {
        case .success:
    ...
    func refreshLogin(completion: ((Result<AccessCredentials>) -> Void)? = nil) {
