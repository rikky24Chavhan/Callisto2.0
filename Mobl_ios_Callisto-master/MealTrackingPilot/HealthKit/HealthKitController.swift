...
public final class HealthKitController: RealmSyncController {
    ...
    func requestAuthorization(completion: ((Result<Bool>) -> Void)? = nil) {
    ...
    func queryCumulativeSum(_ healthKitType: HealthKitType, startDate: Date, intervalComponents: DateComponents, completion: ((Result<Void>) -> Void)? = nil) {
        guard
            isHealthDataAvailable(),
            let sampleType = healthKitType.sampleType
            else {
                completion?(.failure(HealthKitError.healthKitUnavailable))
                return
        }
        ...
        query.initialResultsHandler = { (query, collection, error) in
            ...
            let endDate = startOfCurrentHour.addingTimeInterval(-1)
    ...
    func createObject(_ object: RealmStepSample, completion: ((Result<Void>) -> Void)?) {
